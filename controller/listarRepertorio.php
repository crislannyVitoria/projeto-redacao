...
if ($result->num_rows > 0) {
    ...
    $dados .= "<h2>Eixo Temático: $tema</h2>";

    foreach ($citacoes as $citacao) {
        $dados .="<p><strong>Nome do autor: </strong> " . $citacao["nome_autor"] . "</p>";
        $dados .="<p><strong>Citação: </strong> " . $citacao["citacao"] . "</p>";
        $dados .="<p><strong>Tipo de Repertorio: </strong> " . $citacao["id_tipo_repertorio"] . "</p>";
        $dados .="<p><strong>Data da publicação: </strong> " . $citacao["data_citacao"] . "</p>";
        $dados .="<a class = 'btn btn-primary' onclick='apagarRepertorio($citacao[id_repertorio])'>
